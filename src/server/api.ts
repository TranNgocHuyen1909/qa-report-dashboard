...
import type { BugRecord, ChecklistItem, DashboardFilters, PeriodType } from "../shared/types";
...
export function createApi(deps: ApiDeps) {
  ...
  app.get("/api/dashboard", (req, res) => {
    try {
      ...
      const view = buildDashboardView(deps.getBugs(), filters, deps.getChecklist(), deps.getConclusions());
  ...
  app.post("/api/refresh", (_req, res) => {
    ...
    const view = buildDashboardView(deps.getBugs(), { periodType: "week" }, deps.getChecklist(), deps.getConclusions());
    res.json({ ...view, conclusions: deps.getConclusions(), refreshing: true });
  });

  app.post("/api/conclusions", (req, res) => {
    const { periodKey, good, bad, risks, manDaysOverrides } = req.body;
    if (!periodKey) { res.status(400).json({ error: "periodKey required" }); return; }
  ...
  // Checklist CRUD
  app.get("/api/checklist", (_req, res) => { res.json(deps.getChecklist()); });
  ...
  app.post("/api/checklist/grab", async (req, res) => {
    try {
      ...
      const result = await grabTruongComments(deps.getBugs(), currentList, deps.githubToken);
